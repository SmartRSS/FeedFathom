...
FROM oven/bun:1.2.20-alpine AS dev
WORKDIR /app
ENTRYPOINT ["/usr/local/bin/bun"]

# --- Installer (shared dependencies) ---
FROM --platform=$BUILDPLATFORM oven/bun:1.2.20-alpine AS installer
WORKDIR /app
...
# --- Server Builder ---
FROM --platform=$BUILDPLATFORM oven/bun:1.2.20-alpine AS builder-server
WORKDIR /app
...
# --- Worker Builder ---
FROM --platform=$BUILDPLATFORM oven/bun:1.2.20-alpine AS builder-worker
WORKDIR /app
COPY svelte.config.js vite.config.ts tsconfig.json package.json /app/
...
# --- Server Release ---
FROM oven/bun:1.2.20-alpine AS feedfathom-server
WORKDIR /app
...
CMD ["index.js"]
# --- Worker Release ---
FROM oven/bun:1.2.20-alpine AS feedfathom-worker
WORKDIR /app
RUN apk add --no-cache curl
USER 1000:1000
COPY package.json /app/
