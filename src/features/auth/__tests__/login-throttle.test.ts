...
test("blocks an address grinding one account, and only that pair", async () => {
  ...
  const throttle = new LoginThrottle(redis);

  for (let attempt = 0; attempt < 10; attempt++) {
    // eslint-disable-next-line no-await-in-loop -- Each failure builds on the last.
    await throttle.recordFailure(address, "victim@example.test");
  }

  expect(await throttle.blocked(address, "victim@example.test")).toBe(true);
  ...
  expect(await throttle.blocked(address, "other@example.test")).toBe(false);
  expect(await throttle.blocked("198.51.100.4", "victim@example.test")).toBe(
    false,
  );
});
...
test("blocks an address spraying accounts before any one of them locks", async () => {
  ...
  for (let attempt = 0; attempt < 50; attempt++) {
    // eslint-disable-next-line no-await-in-loop -- Each failure builds on the last.
    await throttle.recordFailure(address, `user${attempt}@example.test`);
  }

  expect(await throttle.blocked(address, "user0@example.test")).toBe(true);
  expect(await throttle.blocked(address, "never-tried@example.test")).toBe(
    true,
  );
  expect(await throttle.blocked("198.51.100.4", "user0@example.test")).toBe(
    false,
  );
});
...
test("a success clears the account counter but not the address budget", async () => {
  ...
  await throttle.clearFailures(address, "user0@example.test");
  ...
  await throttle.recordFailure(address, "user0@example.test");
  expect(await throttle.blocked(address, "never-tried@example.test")).toBe(
    true,
  );
});
