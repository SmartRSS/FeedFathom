...
describe("getDateGroup", () => {
  ...
  const testData = [
    {
      // Earliest date within the last month
      earliestLastMonth: "2022-12-10T00:00:00Z",
      // Earliest date within the last week
      earliestLastWeek: "2023-01-03T00:00:00Z",
      // Earliest date within the last year
      earliestLastYear: "2022-01-10T00:00:00Z",
      // Earliest date older than a year
      earliestOlder: "2022-01-09T24:59:59Z",
      // Earliest date that is today
      earliestToday: "2023-01-10T00:00:00Z",
      // Fixed date for testing
      fixedDate: "2023-01-10T00:00:00Z",
      // Latest date within the last month
      latestLastMonth: "2023-01-02T23:59:59Z",
      // Latest date within the last week
      latestLastWeek: "2023-01-09T23:59:59Z",
      // Latest date within the last year
      latestLastYear: "2022-12-09T23:59:59Z",
    },
    // {
  ...
  for (const dataset of testData) {
    ...
    test("should return 'Within the Last Week' for the latest date within the last week", () => {
      const result = getDateGroup(
        boundaryDates,
        new Date(dataset.latestLastWeek),
      );
    ...
    test("should return 'Within the Last Week' for the earliest date within the last week", () => {
      const result = getDateGroup(
        boundaryDates,
        new Date(dataset.earliestLastWeek),
      );
      expect(result).toBe("Within the Last Week");
    });
    ...
    test("should return 'Within the Last Month' for the latest date within the last month", () => {
      const result = getDateGroup(
        boundaryDates,
        new Date(dataset.latestLastMonth),
      );
    ...
    test("should return 'Within the Last Month' for the earliest date within the last month", () => {
      const result = getDateGroup(
        boundaryDates,
        new Date(dataset.earliestLastMonth),
      );
      expect(result).toBe("Within the Last Month");
    });
    ...
    test("should return 'Within the Last Year' for the latest date within the last year", () => {
      const result = getDateGroup(
        boundaryDates,
        new Date(dataset.latestLastYear),
      );
    ...
    test("should return 'Within the Last Year' for the earliest date within the last year", () => {
      const result = getDateGroup(
        boundaryDates,
        new Date(dataset.earliestLastYear),
      );
      expect(result).toBe("Within the Last Year");
    });
    ...
    test("should return 'Older' for the earliest date older than a year", () => {
      const result = getDateGroup(
        boundaryDates,
        new Date(dataset.earliestOlder),
      );
